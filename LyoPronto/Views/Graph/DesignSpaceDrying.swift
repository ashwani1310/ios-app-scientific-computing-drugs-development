...
struct DSDryingLineChart: View {
    ...
    func getYAxisValues (addValues: AdditionalValuesForDSGraph) -> (Double, Double, Double) {
        var minY = addValues.minTime
        var maxY = addValues.maxTime
        if (minY == maxY) {
            minY -= 5
            maxY += 5
        }
            ...
            ForEach(designSpaceModel.sorted(by: {$0.pressure < $1.pressure}), id: \.id) { value in
                ForEach(value.shelfTemp.indices, id: \.self){ index in
                    LineMark(
                        ...
                        y: .value("\(addValues.description[index])", value.time[index])
                    )
            ...
            if let selectedElement = selectedElement {
                ForEach(selectedElement.shelfTemp.indices, id: \.self){ index in
                    PointMark(
                        ...
                        y: .value("\(addValues.description[index])", selectedElement.time[index])
                    )
                    .symbolSize(100.0)
                    .foregroundStyle(
                        colorList[index]
...
struct DSDryingChartView: View {
    ...
    var body: some View {
        VStack{
            ...
            ZStack {
                GroupBox("Drying Time") {
                    Spacer()
                        .frame(height:50)
                    
                    DSDryingLineChart(selectedElement: $selectedElement, designSpaceModel: designSpaceModel, addValues: addValues)
                            ...
                            GeometryReader { nthGeoItem in
                                if let selectedElement = selectedElement {
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        ForEach(selectedElement.shelfTemp.indices, id: \.self) { index in
                                            Text("\(addValues.shortDesc[index]): \(String(format: "%.2f", selectedElement.time[index])) hr")
                                                .font(.callout)
