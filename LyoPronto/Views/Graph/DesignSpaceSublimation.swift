...
struct DSSublimationLineChart: View {
    ...
    func getYAxisValues (addValues: AdditionalValuesForDSGraph) -> (Double, Double, Double) {
        var minY = addValues.minSublimation
        var maxY = addValues.maxSublimation
        if (minY == maxY) {
            minY -= 5
            maxY += 5
        }
    ...
    var body: some View {
        ...
        Chart {
            ForEach(designSpaceModel.sorted(by: {$0.pressure < $1.pressure}), id: \.id) { value in
                ForEach(value.sublimation.indices, id: \.self){ index in
                    LineMark(
                        x: .value("Chamber Pressure", value.pressure),
                        y: .value("\(addValues.description[index])", value.sublimation[index])
                    )
            ...
            if let selectedElement = selectedElement {
                ForEach(selectedElement.sublimation.indices, id: \.self){ index in
                    PointMark(
                        x: .value("Chamber Pressure", selectedElement.pressure),
                        y: .value("\(addValues.description[index])", selectedElement.sublimation[index])
                    )
                    .symbolSize(100.0)
                    .foregroundStyle(
                        colorList[index]
...
struct DSSublimationChartView: View {
    ...
    var body: some View {
        VStack{
            ...
            ZStack {
                GroupBox("Sublimation Flux") {
                    Spacer()
                        .frame(height:50)
                    
                    DSSublimationLineChart(selectedElement: $selectedElement, designSpaceModel: designSpaceModel, addValues: addValues)
                            ...
                            GeometryReader { nthGeoItem in
                                if let selectedElement = selectedElement {
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        ForEach(selectedElement.sublimation.indices, id: \.self) { index in
                                            Text("\(addValues.shortDesc[index]): \(String(format: "%.2f", selectedElement.sublimation[index])) kg/hr/m\u{00B2}")
                                                .font(.callout)
